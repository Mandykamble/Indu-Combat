...
const TRANSLATIONS = {
  "en-US": {
    ...
    "player1Controls": "Player 1: WASD - Move | Q - Attack | E - Block | Hold Right Mouse - Look Around",
    "player2Controls": "Player 2: Arrows - Move | / - Attack | . - Block | Hold Right Mouse - Look Around",
...
const SplitScreenExplorer = () => {
  ...
  const composer1Ref = useRef(null);
  const composer2Ref = useRef(null);
  ...
  const updatePlayerMovement = (player, keyW, keyS, keyA, keyD, walkAnimRef, keys, obstacles, walkAnimation1, walkAnimation2) => {
    let isMoving = false;
    ...
    if (isMoving) {
      ...
      if (walkAnimRef === 1) walkAnimation1.value += 0.3;
      else walkAnimation2.value += 0.3;

      const currentAnim = walkAnimRef === 1 ? walkAnimation1.value : walkAnimation2.value;
      ...
      leftLeg.rotation.x = Math.sin(currentAnim) * 0.5;
      rightLeg.rotation.x = Math.sin(currentAnim + Math.PI) * 0.5;
      leftArm.rotation.x = Math.sin(currentAnim + Math.PI) * 0.3;
      rightArm.rotation.x = Math.sin(currentAnim) * 0.3;
    } else {
      const leftLeg = player.children[4];
      const rightLeg = player.children[5];
      const leftArm = player.children[2];
    ...
    if (player.userData.attackCooldown > 0) {
      player.userData.attackCooldown--;
    } else if (player.userData.isAttacking) {
      player.userData.isAttacking = false;
    }
  };

  const updateMovement = (obstacles, walkAnimation1, walkAnimation2, keys) => {
    if (!gameOver) {
      updatePlayerMovement(player1Ref.current, 'KeyW', 'KeyS', 'KeyA', 'KeyD', 1, keys, obstacles, walkAnimation1, walkAnimation2);
      updatePlayerMovement(player2Ref.current, 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 2, keys, obstacles, walkAnimation1, walkAnimation2);
    }

    // Handle camera rotation with right mouse button
    if (isRightMouseDownRef.current) {
      camera1Ref.current.rotation.y += mouseXRef.current * 0.02;
      camera1Ref.current.rotation.x = Math.max(-Math.PI/4, Math.min(Math.PI/4, camera1Ref.current.rotation.x + mouseYRef.current * 0.02));
      
      camera2Ref.current.rotation.y += mouseXRef.current * 0.02;
      camera2Ref.current.rotation.x = Math.max(-Math.PI/4, Math.min(Math.PI/4, camera2Ref.current.rotation.x + mouseYRef.current * 0.02));
    }

    const idealCamera1Position = new THREE.Vector3(
      player1Ref.current.position.x,
      player1Ref.current.position.y + 8,
      player1Ref.current.position.z + 15
    );
    camera1Ref.current.position.lerp(idealCamera1Position, 0.05);
    camera1Ref.current.lookAt(player1Ref.current.position.x, player1Ref.current.position.y + 2, player1Ref.current.position.z);

    const idealCamera2Position = new THREE.Vector3(
      player2Ref.current.position.x,
      player2Ref.current.position.y + 8,
      player2Ref.current.position.z + 15
    );
    camera2Ref.current.position.lerp(idealCamera2Position, 0.05);
    camera2Ref.current.lookAt(player2Ref.current.position.x, player2Ref.current.position.y + 2, player2Ref.current.position.z);
  };

  const animate = (obstacles, walkAnimation1, walkAnimation2, keys, sparks) => {
    if (gameOver) return;
    animationRef.current = requestAnimationFrame(() => animate(obstacles, walkAnimation1, walkAnimation2, keys, sparks));
    updateMovement(obstacles, walkAnimation1, walkAnimation2, keys);
  ...
  // Create arena with better graphics
  const createArena = (scene) => {
    ...
    // Add decorative pillars around the arena
    for (let i = 0; i < 16; i++) {
  ...
  useEffect(() => {
    const keys = {};
    const walkAnimation1 = { value: 0 };
    const walkAnimation2 = { value: 0 };
    const sparks = [];
    ...
    composer1.setSize(window.innerWidth / 2, window.innerHeight);
    composer1.addPass(new RenderPass(scene, camera1));
    composer1.addPass(new UnrealBloomPass(
      new THREE.Vector2(window.innerWidth / 2, window.innerHeight),
      0.3,
      0.4,
      0.85
    ));
    composer1Ref.current = composer1;

    const composer2 = new EffectComposer(renderer);
    composer2.setSize(window.innerWidth / 2, window.innerHeight);
    composer2.addPass(new RenderPass(scene, camera2));
    composer2.addPass(new UnrealBloomPass(
      new THREE.Vector2(window.innerWidth / 2, window.innerHeight),
      0.3,
      0.4,
      0.85
    ));
    composer2Ref.current = composer2;
    ...
    const player1 = createPlayer(0x0066cc, true);
    player1.position.set(-10, 2, 0);
    scene.add(player1);
    player1Ref.current = player1;

    const player2 = createPlayer(0xcc0066, false);
    ...
    const handleKeyDown = (e) => {
      keys[e.code] = true;
      if (e.code === 'KeyQ' && player1Ref.current.userData.attackCooldown <= 0) {
        player1Ref.current.userData.isAttacking = true;
        player1Ref.current.userData.attackCooldown = 20;
        player1Ref.current.userData.attackAnimationProgress = 0;
        const distance = player1Ref.current.position.distanceTo(player2Ref.current.position);
        if (distance < 5 && !player2Ref.current.userData.isBlocking) {
          setPlayer2Health(prev => Math.max(0, prev - 20));
          const sparkPos = player2Ref.current.position.clone().add(new THREE.Vector3(0, 2, 0));
          sparks.push(createSpark(sparkPos));
        }
      }
      if (e.code === 'KeyE') {
        player1Ref.current.userData.isBlocking = true;
        player1Ref.current.userData.blockAnimationProgress = 0;
      }
      if (e.code === 'Slash' && player2Ref.current.userData.attackCooldown <= 0) {
        player2Ref.current.userData.isAttacking = true;
        player2Ref.current.userData.attackCooldown = 20;
        player2Ref.current.userData.attackAnimationProgress = 0;
        const distance = player1Ref.current.position.distanceTo(player2Ref.current.position);
        if (distance < 5 && !player1Ref.current.userData.isBlocking) {
          setPlayer1Health(prev => Math.max(0, prev - 20));
          const sparkPos = player1Ref.current.position.clone().add(new THREE.Vector3(0, 2, 0));
          sparks.push(createSpark(sparkPos));
        }
      }
      if (e.code === 'Period') {
        player2Ref.current.userData.isBlocking = true;
        player2Ref.current.userData.blockAnimationProgress = 0;
      }
    };
    ...
    const handleKeyUp = (e) => {
      keys[e.code] = false;
      if (e.code === 'KeyE') {
        player1Ref.current.userData.isBlocking = false;
      }
      if (e.code === 'Period') {
        player2Ref.current.userData.isBlocking = false;
      }
    };
    ...
    animate(obstacles, walkAnimation1, walkAnimation2, keys, sparks);

    const handleResize = () => {
      const width = window.innerWidth;
      const height = window.innerHeight;
      camera1Ref.current.aspect = (width / 2) / height;
      camera1Ref.current.updateProjectionMatrix();
      camera2Ref.current.aspect = (width / 2) / height;
      camera2Ref.current.updateProjectionMatrix();
      renderer.setSize(width, height);
      composer1Ref.current.setSize(width / 2, height);
      composer2Ref.current.setSize(width / 2, height);
    };
  ...
  const restartGame = () => {
    ...
    if (player1Ref.current) {
      player1Ref.current.position.set(-10, 2, 0);
      player1Ref.current.userData.isAttacking = false;
      player1Ref.current.userData.isBlocking = false;
      player1Ref.current.userData.attackCooldown = 0;
      player1Ref.current.userData.attackAnimationProgress = 0;
      player1Ref.current.userData.blockAnimationProgress = 0;
    }
    if (player2Ref.current) {
      player2Ref.current.position.set(10, 2, 0);
      player2Ref.current.userData.isAttacking = false;
      player2Ref.current.userData.isBlocking = false;
      player2Ref.current.userData.attackCooldown = 0;
      player2Ref.current.userData.attackAnimationProgress = 0;
      player2Ref.current.userData.blockAnimationProgress = 0;
    }
    if (animationRef.current) {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {/* Player 1 Health and Controls (Left Viewport) */}
      <div className="health-container player1-ui">
        <h3 className="text-xl font-bold mb-2 text-blue-300">Player 1 (Blue)</h3>
        <p className="text-sm mb-2 text-white">{t('player1Controls')}</p>
        <div className="flex items-center">
          <span className="mr-2 text-sm text-white">Health:</span>
          <div className="health-bar">
            <div className="health-fill" style={{ width: `${player1Health}%` }} />
          </div>
        </div>
      </div>
      
      {/* Player 2 Health and Controls (Right Viewport) */}
      <div className="health-container player2-ui">
        <h3 className="text-xl font-bold mb-2 text-pink-300">Player 2 (Pink)</h3>
        <p className="text-sm mb-2 text-white">{t('player2Controls')}</p>
        <div className="flex items-center">
          <span className="mr-2 text-sm text-white">Health:</span>
          <div className="health-bar">
            <div className="health-fill" style={{ width: `${player2Health}%` }} />
          </div>
        </div>
      </div>
      ...
      {gameOver && (
        <div className="game-over">
          <h2 className="text-3xl font-bold mb-4 text-yellow-300">{winner === 1 ? t('player1Wins') : t('player2Wins')}</h2>
          <button
